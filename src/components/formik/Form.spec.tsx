...
describe('Multi Step Form', () => {
  ...
  it('onSubmit is called when all fields pass validation', async () => {
    user.type(getFirstName(), 'Zourdy');
    selectJobSituation('Full-Time');
    user.type(getUserCity(), 'Wroclaw');
  ...
  it('has 3 required fields on first step', async () => {
    ...
    expect(getUserCity()).toHaveErrorMessage('city is a required field');
  ...
  describe('city field', () => {
    it('shows error when city has less than 5 chars', async () => {
      user.type(getUserCity(), 'Vila');
      user.tab();

      await waitFor(() => {
        expect(getUserCity()).toHaveErrorMessage(
    ...
    it('shows error when city has more than 11 chars', async () => {
      user.type(getUserCity(), 'Vila Real12312313123');
      user.tab();
  ...
  describe('first name field', () => {
    it('shows error when first name has more than 8 chars', async () => {
      user.type(getFirstName(), 'Carlossazazadad');
      user.tab();

      await waitFor(() => {
        expect(getFirstName()).toHaveErrorMessage(
          `Your name can't be longer than 8 chars`
        );
      });
    });
  });
...
//? create function helper for test so our test is more cleaner
